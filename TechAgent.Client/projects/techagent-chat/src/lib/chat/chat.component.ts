import {
  ...
  signal, computed, inject, NgZone, DestroyRef, ViewEncapsulation
} from '@angular/core';
import { DOCUMENT } from '@angular/common';
import { takeUntilDestroyed } from '@angular/core/rxjs-interop';
import { registerLicense } from '@syncfusion/ej2-base';
import { SYNCFUSION_LICENSE_KEY } from '../tokens/api-url.token';
...
export class TechAgentChatComponent implements OnInit, OnDestroy {
  ...
  constructor() {
    const licenseKey = inject(SYNCFUSION_LICENSE_KEY);
    if (licenseKey) registerLicense(licenseKey);

    const doc = inject(DOCUMENT);
    if (!doc.getElementById('sf-techagent-styles')) {
      const link = doc.createElement('link');
      link.id   = 'sf-techagent-styles';
      link.rel  = 'stylesheet';
      link.href = 'https://cdn.syncfusion.com/ej2/33.2.15/material.css';
  ...
  ngOnDestroy(): void { this.streamAbort?.abort(); }
  ...
  onSend(event: SendEvent): void { this.startStream(event.text, event.attachment); }
  ...
  private startStream(text: string, attachment: { name: string; url: string; contentType: string } | null): void {
    ...
    const userMsg: AiMessage = { text, author: this.currentUser, timeStamp: new Date(), attachment };
  ...
  private onStreamError(aiIdx: number, partial: string): void {
    const next = [...this.messages()];
    next[aiIdx] = { ...next[aiIdx], text: partial || 'Connection error. Please check your API URL.' };
    this.messages.set(next);
    this.isLoading.set(false);
  }
}